...
import { AuthProvider } from "../../../auth/dist/standalone/auth.umd.js";
...
async function getJWT() {
  /* Authenticate user via custom verification server*/
  let token = "hello";

  const apiUrl = 'http://localhost:9980/login';
  const data = {
    name: "admin",
    password: "abc"
  };
  ...
  (async () => {
    try {
      const rawResponse = await fetch(apiUrl, {
        ...
        body: JSON.stringify({"name": "admin", "password": "abc"})
      });
      ...
      /* Verify Token: Not essential */

      /*
      const fetchUrl = 'http://localhost:9980/verify'+'?token="+JSON.stringify({"token": token});
      const rResponse = await fetch(fetchUrl, {
          method: 'GET',
          mode: 'cors',
          headers: {
           'Accept': 'application/json',
      ...
      const nContent = await rResponse.json();
      console.log("Content Verify Token:", nContent);
      */

      /* Login with Custom OAuth */
      if(isAuthInit) { 
      const ret = await auth.loginWithCustomProvider("temp", token, "admin");
      console.log("custom oauth:", ret);
...
document.querySelector("#Btn-GetJWT").addEventListener("click", getJWT);
